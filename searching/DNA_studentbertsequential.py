...
class StudentCell(pl.LightningModule):
    def __init__(self, cell, block_num, cell_num, teacher):
        ...
        for layer in self.cell:
            self.model.append(OPS[layer](embed_dim, seq_len, stride=1, affine=False))
        ...
        for idx,layer in enumerate(self.model):
            if idx == 0:
                if layer.__class__.__name__ == 'MultiheadAttention':
                    # transpose the input so that size is [batch_size, seq_len, embed_dim]
...
def generate_cell_encodings(num_blocks, num_cells, num_ops):
    for b in range(num_blocks):
        for c in range(num_cells):
            ...
            saved_dir = os.path.join("searching/cell_encodings_sequential", "block_{}_cell_{}.txt".format(b + 1, c + 1))

            with open(saved_dir, 'w') as f:
                for encoding in cell_encoding:
                    op_decode = ops_dict[encoding]
                    if op_decode == 'pooling':
...
if __name__ == "__main__":
    ...
    saved_dir = os.path.join("searching/cell_encodings_sequential", "{}_block_{}_cell_{}.txt".format(current_time, block_num, cell_num))
    ...
    wandb_logger = WandbLogger(name='{}-block-{}-cell-{}'.format(now, model.block_num, model.cell_num), project='BERT_NAS_sequential_block_{}_cell_{}'.format(block_num, cell_num))
    wandb_logger.experiment.save(saved_dir)
    
    #saves checkpoint at the minimum validation loss
    checkpoint_callback = ModelCheckpoint(mode='min', monitor='val_loss', dirpath='checkpoint_sequential/block_{}_cell_{}/time={}'.format(model.block_num, model.cell_num, now), filename='{epoch}-{val_loss:.2f}')
